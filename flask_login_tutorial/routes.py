...
from .models import Chunk_data, Notebook, Chunk, History, db
...
def get_notebook_id(db, current_user):
    print(current_user)
    notebook_id = db.session.query(func.max(History.notebook_id)).filter_by(user=current_user).first()
    print(notebook_id)
    if not notebook_id:
        notebook_id = History.query.select("max(notebook_id)").first()
...
def get_chunk_id(db, current_user, notebook_id):
    chunk_id = db.session.query(func.max(Chunk.chunk_id)).filter_by(user=current_user, notebook_id=notebook_id).first()[0]
    print(chunk_id)
    if not chunk_id:
        chunk_id = -1
    chunk_id += 1
